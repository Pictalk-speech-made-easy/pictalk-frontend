<template>
	<div class="container is-widescreen">
		<pictoList :isPicto="isPicto" :pictos="loadedCollections" :adminMode="isAdmin" />
	</div>
</template>
<script>
...
export default {
	...
	computed: {
		...
		loadedCollections() {
			console.log("loadedCollections changed !");
		...
		if (collections.length !== 0) {
			...
			try {
				const res = await axios.get(
					"http://localhost:3001/pictalk/collection"
				);
				res.data.map(collection => {
					if (collection.path) {
						collection.path =
							"http://localhost:3001/pictalk/" + collection.path;
					}
				});
